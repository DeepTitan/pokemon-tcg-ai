...
function SpecialConditionIcon({ condition, size, className = 'special-condition-icon' }: { condition: string; size?: number; className?: string }) {
  ...
  switch (condition) {
    case 'Poisoned': return <Skull className={className} size={size} weight="fill" aria-hidden />;
...
export default function TrackerApp() {
  ...
  return (
    <div className={`app-shell ${sharedMode ? 'shared-replay' : ''}`}>
      ...
      {!sharedMode && showSetup && <div className="modal-backdrop"><div className="setup-modal"><div className="modal-title"><div><span>Trace settings</span><h2>Capture and updates</h2></div><button type="button" disabled={busy} onClick={closeSetup} aria-label="Close settings"><X size={21} weight="bold" /></button></div><p>Manage updates and Trace’s connection to TCG Live.</p><div id="settings-updates" /><small className="capture-privacy-disclosure">By connecting, Trace securely sends and stores captured match data, including player names and game actions.</small><div className="modal-actions"><button type="button" disabled={busy} onClick={closeSetup}>Close</button><button className="primary" type="button" disabled={busy} onClick={() => void finishSetup()}>{busy ? 'Working…' : environment.capture.permissionReady ? 'Reconnect capture' : 'Connect capture'}</button></div></div></div>}
      ...
      {!sharedMode && <UpdateNotice matchInProgress={environment.clientRunning} settingsOpen={showSetup} />}
