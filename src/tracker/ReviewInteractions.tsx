...
function CardInspector({ card, pokemon, effects = [], catalog, onInspectCard }: { card: Card; pokemon?: PokemonInPlay; effects?: ReviewAppliedEffect[]; catalog: ReadonlyMap<string, CardInfo>; onInspectCard: (card: Card, pokemon?: PokemonInPlay) => void }) {
  const sourceId = cardSourceIdFromReviewCard(card);
  const catalogCard = sourceId ? catalog.get(sourceId) || catalog.get(sourceId.toLowerCase()) : [...catalog.values()].find((candidate) => candidate.name === card.name);
  const rules = cardRulesText(card, catalogCard);
  return <div className="card-inspector-layout">
    <div className="inspector-art"><CardImage card={card} /><span>{card.cardNumber || 'Captured card'}</span></div>
    <div className="inspector-card-copy">
      <div className="inspector-eyebrow">{card.cardType === CardType.Pokemon ? (card as PokemonCard).stage : card.cardType === CardType.Trainer ? (card as TrainerCard).trainerType : (card as EnergyCard).energySubtype}</div>
      <h2>{card.name}</h2>
      {card.cardType === CardType.Pokemon && pokemon
        ? <PokemonDetails pokemon={pokemon} onInspectCard={onInspectCard} />
        : card.cardType === CardType.Pokemon
          ? <><div className="pokemon-vitals"><span><b>{(card as PokemonCard).hp}</b> HP</span><span>{(card as PokemonCard).stage}</span></div>{(card as PokemonCard).ability && <section className="card-action ability-action"><span>Ability</span><div><strong>{(card as PokemonCard).ability!.name}</strong><p>{(card as PokemonCard).ability!.description}</p></div></section>}{(card as PokemonCard).attacks.map((attack) => <section className="card-action" key={attack.name}><EnergyPips types={attack.cost} /><div><strong>{attack.name}<b>{attack.damage || ''}</b></strong><p>{attack.description}</p></div></section>)}</>
          : card.cardType === CardType.Energy
            ? <EnergyDetails card={card as EnergyCard} rules={rules} />
...
function ZoneInspector({ inspector, onInspectCard }: { inspector: Extract<ReviewInspector, { kind: 'zone' }>; onInspectCard: (card: Card) => void }) {
  ...
  return <>
    ...
    <div className="review-card-grid">{inspector.cards.map((card, index) => {
      ...
      return <button type="button" className={hidden ? 'hidden' : ''} key={`${card.id}-${index}`} disabled={hidden} onClick={() => onInspectCard(card)}><CardImage card={card} hidden={hidden} /><span>{hidden ? 'Unknown card' : card.name}</span></button>;
...
function SelectionInspector({ selection, sourceName, onInspectCard }: { selection: ReviewSelection; sourceName?: string; onInspectCard: (card: Card) => void }) {
  ...
  return <>
    ...
    <div className="review-card-grid selection-grid">{selection.optionCards.map((card, index) => {
      ...
      return <button type="button" key={`${id}-${index}`} className={`${isEligible ? 'eligible' : 'ineligible'} ${isSelected ? 'selected' : ''} ${hidden ? 'hidden' : ''}`} disabled={hidden} onClick={() => onInspectCard(card)}><CardImage card={card} hidden={hidden} />{isSelected && <b><CheckCircle size={15} weight="fill" /> Chosen</b>}<span>{hidden ? 'Private card' : card.name}</span></button>;
