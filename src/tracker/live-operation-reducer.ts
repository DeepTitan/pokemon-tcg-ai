...
function updateZoneCountsFromMove(
  ...
  for (const candidate of list(modification, 'moveCardDeltas', 'MoveCardDeltas')) {
    ...
    if (from === to) continue;
    if (from != null && zoneCounts.has(from)) zoneCounts.set(from, Math.max(0, (zoneCounts.get(from) || 0) - 1));
    if (to != null && zoneCounts.has(to)) zoneCounts.set(to, (zoneCounts.get(to) || 0) + 1);
...
function reconcileHiddenZones(assembly: MatchAssembly, localSide?: 1 | 2): void {
  ...
  for (const pos of REVIEW_ZONE_POSITIONS) {
    ...
    const existing = [...assembly.entities.values()].filter((entity) => number(entity, 'currentGamePos', 'currentPos') === pos).length;
...
export class LiveReviewAssembler {
  ...
  ingest(captured: CapturedOperation): MatchReview | null {
    ...
    const eventRank = (key: string): number => {
      ...
      if (key.startsWith('knockout:')) return 3;
      if (key.startsWith('prize:')) return 4;
      return 5;
    };
