use crate::{privileged, wire::{decode_websocket_message, CapturedOperation}};
...
const CERTIFICATE_VERSION_MARKER: &str = "apple-chain-v3";
...
impl Recorder {
    fn inspect(&self, data: &[u8], host: &str) {
        ...
        if let Some(operation) =
            decode_websocket_message(data, host.to_owned(), unix_timestamp())
        {
...
fn ensure_ca(app: &AppHandle) -> Result<CaptureAuthorityPaths, String> {
    ...
    let root_key = KeyPair::generate().map_err(|error| error.to_string())?;
    ...
    let root_issuer = Issuer::from_ca_cert_pem(&root_pem, root_key)
        .map_err(|error| error.to_string())?;

    let issuer_key = KeyPair::generate().map_err(|error| error.to_string())?;
...
fn trust_domain_contains(
    ...
    let Ok(export_path) = capture_dir(app).map(|path| path.join(export_name))
    else {
        return false;
    };
...
"#;
        ...
        let status = Command::new("/bin/launchctl")
            ...
            .status()
            .map_err(|error| format!("Could not configure TCG Live's system TLS provider: {error}"))?;
        if !status.success() {
...
pub fn request_permission(app: &AppHandle) -> Result<CaptureStatus, String> {
    ...
    if !certificate_ready(app) || !privileged::helper_ready() {
        return Err("macOS did not finish approving the local capture helper.".to_owned());
    }
    Ok(status(app))
}
...
fn server_config(app: &AppHandle) -> Result<Arc<ServerConfig>, String> {
    ...
    let issuer_pem =
        fs::read_to_string(paths.issuer_cert).map_err(|error| error.to_string())?;
    let issuer_key_pem =
        fs::read_to_string(paths.issuer_key).map_err(|error| error.to_string())?;
    let issuer_key = KeyPair::from_pem(&issuer_key_pem).map_err(|error| error.to_string())?;
    let issuer = Issuer::from_ca_cert_pem(&issuer_pem, issuer_key)
        .map_err(|error| error.to_string())?;
            ...
            match socket.connect(&address.into()) {
                Ok(()) => {
                    ...
                    stream
                        ...
                        .map_err(|error| format!("Could not set the upstream socket nonblocking: {error}"))?;
                    return Ok(stream);
                }
                Err(error) => {
        ...
        loop {
            ...
            if payload_length == 126 {
                ...
                payload_length = u16::from_be_bytes([
                    self.websocket_bytes[2],
                    self.websocket_bytes[3],
                ]) as usize;
                header_length = 4;
            ...
            let mut payload = self.websocket_bytes
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let client_to_server = async {
        tokio::io::copy(&mut client_read, &mut server_write)
            .await
            .map_err(|error| format!("Forwarding TCG Live to the Pokémon server failed: {error}"))?;
        server_write
            .shutdown()
            .await
    ...
    let server_to_client = async {
        ...
        loop {
            ...
            client_write
                ...
                .await
                .map_err(|error| format!("Forwarding the Pokémon server to TCG Live failed: {error}"))?;
...
async fn observe_connection(
    ...
    let server = connector
        ...
        .await
        .map_err(|error| format!("Could not establish the upstream Pokémon TLS session: {error}"))?;
    recorder
        .state
        .client_connections
    ...
    *state
    ...
    tauri::async_runtime::spawn(async move {
        if let Err(error) = run_observer(app, observer_state.clone(), listener, shutdown_rx).await {
            ...
            observer_state.observer_running.store(false, Ordering::Relaxed);
        }
    });
    for _ in 0..20 {
        if state.observer_running.load(Ordering::Relaxed) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        while !manager_state.terminate.load(Ordering::Relaxed) {
                ...
                && manager_state.routed_pid.load(Ordering::Relaxed) == u64::from(pokemon_pid)
            {
                ...
                {
                    let started = zero_frame_route_since
        ...
        release_route(&manager_state);
        manager_state.manager_running.store(false, Ordering::Relaxed);
    });
}
